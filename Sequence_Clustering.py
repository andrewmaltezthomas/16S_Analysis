...
subprocess.call("cat " + filtered_seqs + "*.renamed.fna > " + working_directory + "Sequence-Clustering/All.filtered.fna", shell = True)
os.chdir(working_directory + "Sequence-Clustering/")
subprocess.call("perl " + scripts_directory + "Utilities/Qiime2Uparse.pl -i All.filtered.fna -o All.filtered.uparse.fna", shell = True)
...
p3.wait()
p4 = subprocess.Popen(["usearch", "-uchime_ref", "All.filtered.uparse.derep.sorted.otus1.fna", "-db", scripts_directory + "Utilities/gold.fa", "-strand", "plus", "-nonchimeras", "All.filtered.uparse.derep.sorted.otus2.fna"])
p4.wait()
...
subprocess.call("perl " + scripts_directory + "Utilities/otuName.pl -i All.filtered.uparse.derep.sorted.otus2.renamed.fna -o All.filtered.rep.set.otus.fna", shell = True)
...
p7.wait()
p8 = subprocess.Popen(["/usr/bin/qiime", "filter_alignment.py", "-i", "All.filtered.rep.set.otus.Aligned/All.filtered.rep.set.otus_aligned.fasta", "-o", "All.filtered.rep.set.otus.Filtered.Aligned", "-m", scripts_directory + "Utilities/lanemask_in_1s_and_0s"])
p8.wait()
...
p9.wait()
subprocess.call("python " + scripts_directory + "Utilities/map2qiime.py All.filtered.uc > All.filtered.txt", shell = True)
p10 = subprocess.Popen(["/usr/bin/qiime", "make_otu_table.py", "-i", "All.filtered.txt", "-t", "All.filtered.rep.set.otus.Taxonomy/All.filtered.rep.set.otus_tax_assignments.txt", "-o", project + ".otu.table.biom"])
p10.wait()
